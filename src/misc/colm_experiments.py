...
import time
import logging
GCATSL_root ='/Users/yitepeli/PycharmProjects/SL_experiments/src/comparison/GCATSL/'
import argparse

parser = argparse.ArgumentParser(description='Run Feature Generation')
parser.add_argument('--task', '-t', metavar='the-task', dest='task', type=str, help='Choose task', default='GRSMF_crossds_test_gs_trteus')
...
def fit_predict_2set_cc(model, samples_train, samples_test, n_split=5, fold_usage={}):
    if fold_usage:
        fold_ind = coll.OrderedDict()
        train_cancer = list(fold_usage.keys())[0]
        test_cancer = list(fold_usage.keys())[1]
        train_ind_loc = config.DATA_DIR / 'feature_sets' / ('train_test_'+train_cancer+'.json')
        test_ind_loc = config.DATA_DIR / 'feature_sets' / ('train_test_'+test_cancer+'.json')
...
def fit_predict_2set(model, samples_train, train_test_dict):
    #if 'SL' in samples_train.columns.values:
    #    #samples_train = samples_train.rename(columns={'SL': 'class'})
    results = coll.OrderedDict()
    results['best_params']={}
    #samples_train = samples_train.iloc[:,:4]
    #samples_train['pair_name']=samples_train[['gene1', 'gene2']].agg('|'.join, axis=1)
    #samples_train = samples_train.drop(columns=['gene1', 'gene2'])
    char2bin={'Y':1, 'N':0}
    for fold, train_names in train_test_dict.items():
        #train_df = samples_train[samples_train['pair_name'].isin(train_names)].copy()
        #test_df = samples_train[~samples_train['pair_name'].isin(train_names)].copy()
        fold_all_time = time.time()
        #tr_indices = balance_by_index(train_df, np.arange(len(train_df)), rand_seed=124 + int(fold))
        #te_indices = balance_by_index(test_df, np.arange(len(test_df)), rand_seed=124 + int(fold))
        #sample_dict = {'train':tr_indices, 'test':te_indices}

        train_df = train_names['train'].rename(columns={'SL': 'class'}).replace({'class':char2bin})
        test_df = train_names['test'].rename(columns={'SL': 'class'}).replace({'class':char2bin})
        train_df = train_df.iloc[:, :4]
        test_df = test_df.iloc[:, :4]
        train_df['pair_name'] = train_df[['gene1', 'gene2']].agg('|'.join, axis=1)
        train_df = train_df.drop(columns=['gene1', 'gene2'])
        test_df['pair_name'] = test_df[['gene1', 'gene2']].agg('|'.join, axis=1)
        test_df = test_df.drop(columns=['gene1', 'gene2'])
        sample_dict = {'train': np.arange(len(train_df)), 'test': np.arange(len(test_df))}

        fold_start_time = time.time()
        results[fold] = model.fit_predict_all_datasets_2sets(sample_dict, train_df, test_df, cv=int(fold)-1)
...
def fit_predict_dho_cross_validation(model, samples_train, train_test_dict):
    results = coll.OrderedDict()
    results['best_params']={}
    char2bin={'Y':1, 'N':0}
    for fold, train_names in train_test_dict.items():
        ...
        fold_start_time = time.time()
        results[fold] = model.fit_predict_all_datasets_2sets(sample_dict, train_df, test_df, fold)
...
def single_cancer_test_experiment(data_choice_list, cancer, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results_colm' / model_type / 'single_cancer_test.csv'
    config.ensure_dir(res_loc)
    ...
    if model_type=='GRSMF':
        ...
        model_loc = data_choices + '_' + cancer + '_' + str(grid_search) + '_' + balance_strategy+ '.pickle'
    ...
    model_loc = config.ROOT_DIR / 'results_colm' / model_type / 'models_test' / model_loc
    config.ensure_dir(model_loc)
    if os.path.isfile(model_loc):
        results = dfnc.load_pickle(model_loc)
    else:
        colm_data_loc = loc_dict['combined_original_data_loc']
        all_samples = dfnc.load_rdata_file(colm_data_loc, isRDS=True, is_abs_loc=True)
        all_samples = all_samples.fillna(0)
        all_samples = all_samples[all_samples['cancer_type']==cancer]

        if 'GCATSL' in task:
            model.args_dict['input_dir'] = GCATSL_root+'data/SL_Colm_'+cancer+'/'
            model.args_dict['output_dir'] = GCATSL_root+'output/SL_Colm_'+cancer+'/'
            model.args_dict['log_dir'] = GCATSL_root+'logs/SL_Colm_'+cancer+'/'
            model.args_dict['id2name_dir'] = config.DATA_DIR / 'feature_sets' / 'GCATSL_colm' / cancer
            ...
            model.args_dict['id2name_dir'] = config.DATA_DIR / 'feature_sets' / 'GRSMF_colm' / cancer
        ...
        #train_test_sample_names = dfnc.load_pickle(loc_dict['train_test_names_loc'])
        fold_loc = config.DATA_DIR / 'feature_sets' / (cancer+'_forcolm_single_folds.Rdata')
        train_test_sample_names = dfnc.load_rdata_file(fold_loc, isRDS=True, is_abs_loc=True)
        train_test_sample_names = dict(zip(train_test_sample_names.names, list(train_test_sample_names)))
        for fold_name, fold_item in train_test_sample_names.items():
            train_test_sample_names[fold_name] = dict(zip(fold_item.names, list(fold_item)))
        #fold_predictions = elrf.fit_predict_all_datasets_fold(train_test_sample_dict=train_test_sample_names[cancer])
        results = fit_predict_2set(model, all_samples, train_test_dict=train_test_sample_names)
...
def cross_dataset_test_experiment(data_choice_list, cancer, train_ds, test_ds, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results_colm' / model_type / 'cross_dataset_test.csv'
    config.ensure_dir(res_loc)
    grid_search_settings_loc = config.ROOT_DIR / 'results' / model_type / 'result.json'
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
    ...
    if model_type=='GCATSL':
        model = GCATSL(random_state=124, n_jobs=- 1,
                 ...
                 balance_strategy=balance_strategy, return_model=False, pretrained_model=None, cancer=cancer,
                   feature_dim=feature_dim, n_feature= n_feature, dataset_name = data_choices)
        model_loc = data_choices + '_' + cancer + '_' + train_ds + '_' + test_ds + '_' + str(grid_search) + '_' + balance_strategy + '_' + str(feature_dim) + '.pickle'
    if model_type=='GRSMF':
        model = GRSMF(random_state=124, n_jobs=- 1,
                 verbose=False, thold=0.5, process=True, grid_searched=False, sep_train_test=False, balance=True,
                 balance_strategy=balance_strategy, return_model=False, cancer=cancer,  dataset_name = data_choices)
        model_loc = data_choices + '_' + cancer + '_' + train_ds + '_' + test_ds + '_' + str(grid_search) + '_' + balance_strategy+ '.pickle'
    elif model_type=='ELRRF' or model_type=='ELGBDT' :
        use_comb = False
        use_all_comb = True
        model = ELRRF(boosting_type=boosting, use_single=use_single, grid_searched=grid_search, balance_strategy=balance_strategy,
    ...
    model_loc = config.ROOT_DIR / 'results_colm' / model_type / 'models_cross_ds' / model_loc
    config.ensure_dir(model_loc)
    if os.path.isfile(model_loc):
        results = dfnc.load_pickle(model_loc)
    else:
        ...
        if 'GCATSL' in task:
            model.args_dict['input_dir'] = GCATSL_root+'data/SL_Colm_partial_'+cancer+'/'
            model.args_dict['output_dir'] = GCATSL_root+'output/SL_Colm_partial_'+cancer+'/'
            model.args_dict['log_dir'] = GCATSL_root+'logs/SL_Colm_partial_'+cancer+'/'
            model.args_dict['id2name_dir'] = config.DATA_DIR / 'feature_sets' / 'GCATSL_colm' / cancer
            config.ensure_dir(model.args_dict['input_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['output_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['log_dir']+'/deneme.csv')
            ...
            if best_params:
                model_dict['best_params'] = best_params
        #train_test_sample_names = dfnc.load_pickle(loc_dict['train_test_names_loc'])
        fold_loc = config.DATA_DIR / 'feature_sets' / (cancer+'_'+train_ds+'_'+test_ds+'_'+'forcolm_cd_folds.Rdata')
        train_test_sample_names = dfnc.load_rdata_file(fold_loc, isRDS=True, is_abs_loc=True)
        train_test_sample_names = dict(zip(train_test_sample_names.names, list(train_test_sample_names)))
        for fold_name, fold_item in train_test_sample_names.items():
            train_test_sample_names[fold_name] = dict(zip(fold_item.names, list(fold_item)))
...
def single_cancer_sho_test_experiment(data_choice_list, cancer, grid_search, thold=None):
    model_type='ELRRF'
    comb_type='type2'
    boosting='rf'
    if 'GCATSL' in task:
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results_colm' / model_type / 'single_cancer_sho_test.csv'
    config.ensure_dir(res_loc)
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
        if model_type=='GCATSL':
    ...
    model_loc = config.ROOT_DIR / 'results_colm' / model_type / 'models_sho_test' / model_loc
    config.ensure_dir(model_loc)
    if os.path.isfile(model_loc):
        results = dfnc.load_pickle(model_loc)
    else:
        ...
        if 'GCATSL' in task:
            param_list = ['args_dict','sep_train_test', 'balance', 'balance_strategy', 'cancer' ]
            model.args_dict['input_dir'] = GCATSL_root+'data/SL_Colm_sho_'+cancer+'/'
            model.args_dict['output_dir'] = GCATSL_root+'output/SL_Colm_sho_'+cancer+'/'
            model.args_dict['log_dir'] = GCATSL_root+'logs/SL_Colm_sho_'+cancer+'/'
            model.args_dict['id2name_dir'] = config.DATA_DIR / 'feature_sets' / 'GCATSL_colm' / cancer
            config.ensure_dir(model.args_dict['input_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['output_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['log_dir']+'/deneme.csv')
        ...
        #train_test_sample_names = dfnc.load_pickle(loc_dict['train_test_names_loc'])
        fold_loc = config.DATA_DIR / 'feature_sets' / (cancer+'_single_forcolm_sho_folds.Rdata')
        train_test_sample_names = dfnc.load_rdata_file(fold_loc, isRDS=True, is_abs_loc=True)
        train_test_sample_names = dict(zip(train_test_sample_names.names, list(train_test_sample_names)))
        for fold_name, fold_item in train_test_sample_names.items():
            train_test_sample_names[fold_name] = dict(zip(fold_item.names, list(fold_item)))
        ...
        #fold_predictions = elrf.fit_predict_all_datasets_fold(train_test_sample_dict=train_test_sample_names[cancer])
        #results = fit_predict_2set(model, all_samples, train_test_dict=train_test_sample_names)
        results = fit_predict_dho_cross_validation(model, all_samples, train_test_dict=train_test_sample_names)
        selected_params = {}
        all_params = model.get_params(deep=False)
        for key in param_list:
            selected_params[key] = all_params.pop(key)
...
def single_cancer_dho_test_experiment(data_choice_list, cancer, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results_colm' / model_type / 'single_cancer_dho2_test.csv'
    config.ensure_dir(res_loc)
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
        if model_type=='GCATSL':
    ...
    model_loc = config.ROOT_DIR / 'results_colm' / model_type / 'models_dho2_test' / model_loc
    config.ensure_dir(model_loc)
    if os.path.isfile(model_loc):
        results = dfnc.load_pickle(model_loc)
    else:
        ...
        if 'GCATSL' in task:
            param_list = ['args_dict','sep_train_test', 'balance', 'balance_strategy', 'cancer' ]
            model.args_dict['input_dir'] = GCATSL_root+'data/SL_Colm_dho_'+cancer+'/'
            model.args_dict['output_dir'] = GCATSL_root+'output/SL_Colm_dho_'+cancer+'/'
            model.args_dict['log_dir'] = GCATSL_root+'logs/SL_Colm_dho_'+cancer+'/'
            model.args_dict['id2name_dir'] = config.DATA_DIR / 'feature_sets' / 'GCATSL_colm' / cancer
            config.ensure_dir(model.args_dict['input_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['output_dir']+'/deneme.csv')
            config.ensure_dir(model.args_dict['log_dir']+'/deneme.csv')
        ...
        #train_test_sample_names = dfnc.load_pickle(loc_dict['train_test_names_loc'])
        fold_loc = config.DATA_DIR / 'feature_sets' / (cancer+'_single_forcolm_dho_folds.Rdata')
        train_test_sample_names = dfnc.load_rdata_file(fold_loc, isRDS=True, is_abs_loc=True)
        train_test_sample_names = dict(zip(train_test_sample_names.names, list(train_test_sample_names)))
        for fold_name, fold_item in train_test_sample_names.items():
            train_test_sample_names[fold_name] = dict(zip(fold_item.names, list(fold_item)))
...
def cross_cancer_test_experiment(data_choice_list, cancer_train, cancer_test, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results' / model_type / 'cross_cancer_train_train_test_train.csv'
    config.ensure_dir(res_loc)
    grid_search_settings_loc = config.ROOT_DIR / 'results' / model_type / 'result.json'
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
    ...
    model_loc = config.ROOT_DIR / 'results' / model_type / 'models_cc_train_train_test_train' / model_loc
    config.ensure_dir(model_loc)
    if os.path.isfile(model_loc):
        results = dfnc.load_pickle(model_loc)
    else:
        ...
        for data_choice in data_choice_list:
            ...
            processed_data_te = prepare_cancer_dataset(tr_data, cancer=cancer_test)
        ...
        results = fit_predict_2set_cc(model, samples_tr, samples_te, n_split=n_split,
                                      fold_usage={cancer_train:'train', cancer_test:'train'})
        param_list = ['boosting_type', 'num_leaves', 'max_depth', 'learning_rate', 'n_estimators',
                      'subsample_for_bin', 'objective', 'class_weight', 'min_split_gain',
                      'min_child_weight', 'min_child_samples', 'subsample', 'subsample_freq',
                      'colsample_bytree', 'reg_alpha', 'reg_lambda', 'random_state', 'n_jobs',
...
def multi_cross_cancer_test_experiment(data_choice_list, cancer_trains, cancer_test, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results' / model_type / 'multi_cross_cancer_train_train_test_train_nocontr.csv'
    config.ensure_dir(res_loc)
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
        chosen_cols = result_df[
    ...
    for cancer_train in cancer_trains:
        model_loc = data_choices + '_' + cancer_train + '_' + cancer_test + "_" + str(grid_search) + '_' + str(
            use_single) + '_' + comb_type + '_' + str(n_split) + \
                    '_' + balance_strategy + '.pickle'
        contr_loc = config.ROOT_DIR / 'results' / model_type / 'models_cc_train_train_test_test' / model_loc
        model_loc = config.ROOT_DIR / 'results' / model_type / 'models_cc_train_train_test_train' / model_loc
...
def single_cancer_dataset_imp_detail_experiment(data_choice_list, cancer, grid_search, thold=None):
    ...
    loc_dict = get_locs()
    res_loc = config.ROOT_DIR / 'results' / model_type / ('single_cancer_'+cancer+'_set_imp_test2.csv')
    config.ensure_dir(res_loc)
    '''
    if os.path.isfile(res_loc):
        result_df = pd.read_csv(res_loc)
...
def get_locs():
    ...
    loc_dict['train_test_names_loc'] = config.DATA_DIR / 'labels/Exp1.2.train_test_names.pickle'
    loc_dict['combined_original_data_loc'] = config.DATA_DIR / ('labels/combined.RData')
...
def get_data_choices(seq=None, singles=False, onehot=False, dep_comb=False, all_comb=False, final_comb=False,
                     ...
                     chosen_singles=False, only_seq=False, seq_colm=False, extra_single=False, colm=False):
    if seq==None:
...
def main_cross_ds_test(choices, thold):
    ...
    #cancers = ['KIRC', 'CESC']
    use_single = True
    train_test_dict = {'BRCA': ['isle', 'discoversl'], 'LUAD': ['discoversl', 'isle']}
    gs=False
    if 'gs' in task:
        gs=True
    for tr_cancer in [args.cancer]:
...
def main_sho_cancer_test(choices, thold):
    all_cancers = ['BRCA', 'LUAD', 'OV', 'COAD']#
    #cancers = ['BRCA', 'COAD', 'LAML', 'LUAD', 'OV']
    ...
    for cancer in canc_list:
        for choice_list in choices:
            single_cancer_sho_test_experiment(data_choice_list=choice_list, cancer=cancer, grid_search=gs, thold=thold)
...
def main():
    ...
    for thold in [0.5]:#0.4, 0.425, 0.45, 0.5, 0.525, 0.55, 0.575, 0.6, 0.625, 0.65, 0.7, 0.75, 0.8]:
        for seq in seqs:
            ...
            elif 'sho' in task and 'test' in task:
                main_sho_cancer_test(choices, thold)
