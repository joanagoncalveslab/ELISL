...
parser.add_argument('--exp_file', '-ef', metavar='the-experiment_file', dest='exp_file', type=str, help='Choose experiment description file', default=config.DATA_DIR/"experiment_configs"/"single_cancer_config.json")

args = parser.parse_args()
print(f'Running args:{args}')
task = f'SingleCancer_{args.model}_{args.cancer}_{args.grid_search}_{args.balance_strategy}'
...
def single_cancer_test_experiment():
    ...
    res_loc = config.ROOT_DIR / 'results' / model_type / 'single_cancer_experiment.csv'
    ...
    model_loc = config.ROOT_DIR / 'results' / model_type / 'models_single_cancer' / model_loc
    ...
    if os.path.isfile(model_loc):
        ...
        results = experiment.fit_predict_2set(model, samples_tr, samples_te, n_split=n_split)
    ...
    logging.info(f'Experiment with {data_choice_list} finished for {cancer}...')
    print()

def main():
    single_cancer_test_experiment()
